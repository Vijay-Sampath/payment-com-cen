...
# ---------------------------------------------------------------------------
# Repair Queue Generator
# ---------------------------------------------------------------------------

EXCEPTION_TYPES = [
    {
        "type": "RJCT08_BIC_INVALID",
        "severity": "critical",
        "maker_checker": "FOUR_EYES",
        "root_cause_tpl": "Destination BIC DEST{to}XX failed SWIFT directory validation — likely stale BIC after counterparty migration",
        "fields": [
            {"field": "Destination BIC", "original_tpl": "DEST{to}XX", "proposed_tpl": "{bic_prefix}{to}2L", "source": "SWIFT Directory 2024-Q1 + historical payment graph"},
            {"field": "Receiver Agent", "original": "NOTPROVIDED", "proposed_tpl": "{bic_prefix}{to}2L", "source": "Counterparty mapping database"},
        ],
        "confidence": 0.94,
        "reasoning_tpl": "Cross-referenced SWIFT gpi directory (2024-Q1) with 6 months of successful payment history for {client}. The destination BIC was migrated by the counterparty on 2024-01-10. Proposed BIC has 847 successful settlements in the past 90 days.",
        "reg_notes": "BIC substitution requires FOUR_EYES review per CPMI-IOSCO Principle 17 (Operational Risk). Counterparty verification log attached.",
        "est_time": "< 2 min after approval",
    },
    {
        "type": "NAMC_NAME_MISMATCH",
        "severity": "high",
        "maker_checker": "FOUR_EYES",
        "root_cause_tpl": "Beneficiary name contains unstructured characters — SWIFT gpi name matching failed on special characters in registered name",
        "fields": [
            {"field": "Beneficiary Name", "original": "Bénéficiary Trdg. Co., Ltd.", "proposed": "Beneficiary Trading Co Ltd", "source": "SWIFT name-matching algorithm + counterparty registry"},
            {"field": "Name Matching Override", "original": "STRICT", "proposed": "FUZZY_MATCH_VERIFIED", "source": "Historical match pattern analysis"},
        ],
        "confidence": 0.87,
        "reasoning_tpl": "Name normalization applied: removed diacritics, expanded abbreviations. The counterparty has 23 successful payments under both name variants. Fuzzy match score: 94.2%.",
        "reg_notes": "Name mismatch override requires dual approval per AML/CFT screening guidelines. Sanctions re-check will be triggered automatically post-repair.",
        "est_time": "< 1 min after approval",
    },
    {
        "type": "RJCT11_ADDRESS_UNSTRUCTURED",
        "severity": "high",
        "maker_checker": "FOUR_EYES",
        "root_cause_tpl": "CBPR+ structured address validation rejected unstructured postal address in Field 59 — ISO 20022 migration gap",
        "fields": [
            {"field": "Beneficiary Address", "original": "123 Main St, Suite 400, London", "proposed": "StrtNm: Main St | BldgNb: 123 | Flr: 4 | TwnNm: London | Ctry: GB", "source": "ISO 20022 address parser + Royal Mail PAF database"},
            {"field": "Address Format", "original": "UNSTRUCTURED", "proposed": "STRUCTURED_ISO20022", "source": "CBPR+ validation rules v3.1"},
        ],
        "confidence": 0.89,
        "reasoning_tpl": "Parsed unstructured address into ISO 20022 structured components. Validated against Royal Mail PAF database. Match confidence 96.1%. {client} has 14 prior successful payments to this address.",
        "reg_notes": "Address restructuring under FOUR_EYES review. CBPR+ compliance verified. No sanctions implications.",
        "est_time": "< 1 min after approval",
    },
    {
        "type": "DUPL_DUPLICATE_DETECTED",
        "severity": "medium",
        "maker_checker": "AUTONOMOUS_WITH_AUDIT",
        "root_cause_tpl": "{legacy} retry storm generated duplicate message — original PMT-0047 and retry have identical UETR prefix",
        "fields": [
            {"field": "Duplicate Status", "original": "HELD_PENDING_REVIEW", "proposed": "CANCELLED_DUPLICATE", "source": "UETR correlation + message sequence analysis"},
            {"field": "Original Reference", "original": "NOT_LINKED", "proposed": "PMT-0047 (settled at 14:25:03Z)", "source": "Settlement confirmation database"},
        ],
        "confidence": 0.91,
        "reasoning_tpl": "UETR prefix analysis confirms this payment is a retry duplicate of PMT-0047 generated by the {legacy} retry storm. PMT-0047 was successfully settled at 14:25:03Z. Cancelling the duplicate poses zero settlement risk.",
        "reg_notes": "Duplicate cancellation under AUTONOMOUS_WITH_AUDIT — full evidence chain logged. No value movement involved.",
        "est_time": "Immediate (auto-apply eligible)",
    },
]


def generate_repair_queue(bank_id: str) -> list[dict]:
    """Generate 3-5 residual exception items with AI repair proposals for a bank."""
    bp = BANK_PROFILES[bank_id]
    bic = bp["bic_family"][0]
    bic_prefix = bic[:4]
    clients = bp["client_archetypes"]
    corridors = bp["corridors_primary"]
    legacy = bp["legacy_system"]

    # Pick 3-4 exception types for this bank
    n_items = random.randint(3, 4)
    selected = random.sample(EXCEPTION_TYPES, min(n_items, len(EXCEPTION_TYPES)))

    items = []
    base_time = datetime(2024, 1, 15, 14, 31, 0, tzinfo=timezone.utc)

    for idx, exc in enumerate(selected):
        corridor = corridors[idx % len(corridors)].split("-")
        from_ccy = corridor[0] if len(corridor) == 2 else "USD"
        to_country = corridor[1] if len(corridor) == 2 else "GB"
        client = clients[idx % len(clients)]
        raised_at = base_time + timedelta(seconds=idx * 33)
        proposed_at = raised_at + timedelta(seconds=6)

        # Build field diffs
        fields = []
        for f in exc["fields"]:
            original = f.get("original", f.get("original_tpl", "").format(to=to_country, bic_prefix=bic_prefix))
            proposed = f.get("proposed", f.get("proposed_tpl", "").format(to=to_country, bic_prefix=bic_prefix))
            fields.append({
                "field": f["field"],
                "original": original,
                "proposed": proposed,
        ...
        reasoning = exc["reasoning_tpl"].format(client=client, legacy=legacy)
        root_cause = exc["root_cause_tpl"].format(to=to_country, legacy=legacy)

        amount = round(random.uniform(2_000_000, 30_000_000), -5)

        items.append({
            "id": f"RPR-{bank_id}-{idx + 1:03d}",
            "payment_id": f"PMT-{140 + idx:04d}",
            "uetr": str(uuid.uuid4()),
            ...
            "currency": from_ccy,
            "originator": client,
            "beneficiary": f"Beneficiary-{to_country}-{140 + idx}",
            "corridor": f"{from_ccy}→{to_country}",
            "root_cause": root_cause,
            "ai_proposal": {
                "id": f"PROP-{bank_id}-{idx + 1:03d}",
                "proposed_at": proposed_at.isoformat(),
                "fields": fields,
                "confidence": exc["confidence"],
                "reasoning": reasoning,
                "regulatory_notes": exc["reg_notes"],
                "estimated_time": exc["est_time"],
            },
            "status": "ai_proposed",
            "maker_checker": exc["maker_checker"],
        })
...
def main():
    ...
    with Progress(
        ...
        for bank_id in banks:
            ...
            # 8. Repair queue JSON
            repair_queue = generate_repair_queue(bank_id)
            with open(scenario_dir / "repair_queue.json", "w") as f:
                json.dump(repair_queue, f, indent=2)
            console.print(f"    ✓ {scenario_dir / 'repair_queue.json'} ({len(repair_queue)} items)")

            # 9. Bank profile JSON (written to generator-profiles to avoid overwriting frontend JSONs)
